...
from datasets.syns_patches import EvalDataset
...
class Predictor(object):
    ...
    def _build_dataloader(self) -> DataLoader:
        self._dataset = EvalDataset(self._data_path, self._split)
    ...
    def __call__(self) -> Any:
        ...
        with torch.no_grad():
            for data in self._dataloader:
                ...
                input_color = resize(input_color, (192, 640), interpolation=InterpolationMode.BILINEAR)
        ...
        pred_disps_arr = np.stack(pred_disps, axis=0)
        output_path = Path(self._output_path) / 'submission.zip'
        with ZipFile(output_path, 'w') as zf:
            with zf.open('pred.npz', 'w') as f:
                np.savez_compressed(f, pred=pred_disps_arr)
